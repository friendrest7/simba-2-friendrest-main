    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="mt-4 grid gap-8 lg:grid-cols-2">
        ...
        <div className="flex flex-col">
          ...
          <span
            className={`mt-4 inline-flex w-fit items-center gap-1.5 rounded-full px-3 py-1 text-xs font-semibold ${branchStock > 0 ? "bg-success/15 text-success" : "bg-destructive/15 text-destructive"}`}
          >
          ...
          <div className="mt-6 flex flex-col gap-3">
            {qty > 0 && (
              <div className="flex h-12 w-48 items-center gap-3 rounded-full border border-primary/40 bg-primary/8 px-4">
                <button
                  ...
                  className="rounded-full p-1 text-primary transition-colors enabled:hover:bg-white/15 disabled:text-muted-foreground"
                ...
                <button
                  onClick={() => add(product)}
                  className="rounded-full p-1 text-primary transition-colors enabled:hover:bg-white/15"
            ...
            {qty === 0 && (
              <Button
                ...
                className="h-12 rounded-full gradient-brand px-6 text-brand-foreground hover:opacity-90 gap-2 glow-primary"
              >
            ...
            {showDetails && (
              <div className="mt-3 rounded-xl border border-border bg-card p-4 text-sm text-muted-foreground">
                <div className="grid gap-3 sm:grid-cols-2">
                  <DetailRow label={t("ui.productName")} value={product.name} />
                  <DetailRow label={t("ui.category")} value={categoryLabel(product.category, t)} />
                  <DetailRow label={t("ui.price")} value={formatRWF(product.price)} />
                  <DetailRow label={t("ui.weight")} value={product.unit} />
                  <DetailRow
                    label={t("ui.stockStatus")}
                    value={branchStock > 0 ? t("card.inStock") : t("card.outOfStock")}
                  />
                  <DetailRow label={t("ui.sku")} value={String(product.id)} />
                </div>
                <div className="mt-4 rounded-2xl bg-secondary/50 p-4 text-foreground">
          ...
          <div className="mt-6 grid grid-cols-1 gap-3 sm:grid-cols-3">
            ...
            <Feature
              ...
              desc={t("feature.secureDesc")}
            />
            <Feature
              icon={<Package className="h-4 w-4" />}
              title={t("feature.authentic")}
              desc={t("feature.authenticDesc")}
            />
          </div>
          ...
          <div className="mt-6 rounded-2xl border border-border bg-card p-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <div className="font-semibold text-foreground">{t("product.deliveryTitle")}</div>
                <div className="mt-1 text-sm text-muted-foreground">
                  {t("product.deliveryBody")}
                </div>
              </div>
            </div>
          </div>
...
function DetailRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-xl border border-border/80 bg-background/70 p-3">
      <div className="text-xs font-bold uppercase tracking-[0.12em] text-primary">{label}</div>
